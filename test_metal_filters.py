...
def test_comparison_ui_has_global_metal_controls_and_purity_section():
    source = Path(__file__).with_name("streamlit_app.py").read_text(encoding="utf-8")
    assert '"comparison_metal_silver"' in source
    assert '"comparison_metal_gold"' in source
    assert '"comparison_metal_other"' in source
