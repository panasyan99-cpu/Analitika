...
PRODUCT_FEATURES: tuple[ProductFeature, ...] = (
    ...
    ProductFeature(
        ...
        mode="Заказ Sonu",
        description=(
            "Сетевая аналитика нового отчета Sonu: общий остаток берется один раз на SKU, "
            "а продажи суммируются по магазинам. Для изделий, кроме браслетов, показываются "
            "группа и вид камня, категория, количество SKU, остаток, продажи и потребность на "
            "30/45/90 дней. Браслеты разделяются на модели с центральной композицией и полный "
            "круг. Основные списки переключаются между таблицами и карточками; доступна полная "
            "Excel-выгрузка и контроль расхождений остатка."
        ),
    ),
    ProductFeature(
