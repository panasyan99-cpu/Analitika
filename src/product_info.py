...
PRODUCT_FEATURES: tuple[ProductFeature, ...] = (
    ...
    ProductFeature(
        ...
        description=(
            "Пятый рабочий модуль формирует отдельные заказы по камням и жемчугу, показывает "
            "прозрачные рекомендации, сохраняет Limited Order и размеры колец. Облачное меню "
            "«Незавершённые заказы» получает компактный индекс из Cloudflare R2, показывает даты, "
            "этап, количество изделий и SKU, восстанавливает исходный Excel без ручного JSON и "
            "позволяет полностью удалить заказ вместе со всеми облачными и локальными данными. "
            "В карточке общий остаток считается как «Всего минус магазин 20», а TT и 63 "
            "остаются внутри общего значения и показываются отдельной детализацией."
        ),
    ),
    ProductFeature(
