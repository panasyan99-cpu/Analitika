...
PRODUCT_FEATURES: tuple[ProductFeature, ...] = (
    ...
    ProductFeature(
        ...
        description=(
            ...
            "Он читает ручные комплекты, сохраняет полный состав Set# выбранного камня, "
            "использует фактический NTR2 или fallback для старого файла, фильтрует комплекты по ТВП "
            "без дополнительного раскрытия, очищает старый черновик при загрузке нового отчёта, "
            "распределяет размеры колец и выгружает поставщику Excel с фотографиями и конкретными камнями."
        ),
    ),
    ProductFeature(
